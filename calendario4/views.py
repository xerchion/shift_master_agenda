...
def necessary_team(func):
    def wrapper(request, *args, **kwargs):
        ...
        my_user = UserAdapter(request.user.id).my_user
        print(my_user.team, "----------------------------------")
...
def config(request):
    ...
    if request.method == "POST":
        # TODO aqui puedes hacer un Method Factory, get_form, y en los parametros, en concreto
        # de request puedes obtener el nombre de la vista y saber así que formulario dar.
        form = UserAdapter(id).get_config_form(request.POST)
        if form.is_valid():
            form.save()
            return redirect("agenda")
    else:
        form = UserAdapter(id).get_config_form()
...
def change_pass(request):
    id = request.user.id
    if request.method == "POST":
        form = UserAdapter(id).get_pass_form(request.POST)
        if form.is_valid():
            form.save()
            return redirect("login")
    else:
        form = UserAdapter(id).get_pass_form()
...
def change_color_days(request):
    ...
    if request.method == "POST":
        ...
        if user.color_saved:
            form = user.get_color_form(user.color_saved)
        else:
            form = user.get_color_form()
